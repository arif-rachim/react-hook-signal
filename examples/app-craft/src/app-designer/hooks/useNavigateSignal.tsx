...
export function useNavigateSignal() {
    const {allPagesSignal,activePageIdSignal,allErrorsSignal,uiDisplayModeSignal,variableInitialValueSignal} = useContext(AppDesignerContext)
            ...
            navigate[page.name] = async (param:unknown) => {
                if(uiDisplayModeSignal.get() === 'design'){
                    const go = await showModal(closePanel => {
                        return <div style={{display:'flex',flexDirection:'column',padding:20,maxWidth:300,gap:10}}>
                            ...
                            <div style={{display:'flex',flexDirection:'row',gap:10,justifyContent:'flex-end'}}>
                                <Button onClick={() => closePanel(true)} style={{display:'flex',flexDirection:'row',alignItems:'center',gap:5}}>
                                    <div>Yes</div>
                                    <div style={{display:'flex',alignItems:'center',justifyContent:'center'}}><Icon.Exit/></div>
                                </Button>
                                <Button onClick={() => closePanel(false)}
                                        style={{display: 'flex', flexDirection: 'row',alignItems:"center",gap:5}}>
                                    <div>No</div>
                                    <div style={{display:'flex',alignItems:'center',justifyContent:'center'}}><Icon.ArrowDown/></div>
                                </Button>
                            </div>
                        </div>
                    });

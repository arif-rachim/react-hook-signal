...
export function PageSelector(props: {
    ...
    const elements = useComputed(() => {
        ...
        return pages.filter(i => i.id !== pageToFilterOut).map((i) => {
            ...
            return <div key={i.id} style={{display: 'flex', alignItems: 'center', gap: 5,width:'33.33%'}} onClick={() => {
                const selected = selectedPage.get();
    ...
    return <div style={{display: 'flex', flexDirection: 'column', gap: 10,width:600,height:300}}>
        <div style={{borderBottom:BORDER,padding:20,backgroundColor:'rgba(0,0,0,0.05)',display:'flex',alignItems:'center',gap:5}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',fontSize:22}}><Icon.Page/></div>
